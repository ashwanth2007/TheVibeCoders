...
export const CodeDisplay: React.FC<CodeDisplayProps> = ({ files, onFilesChange, onUndo, onRedo, canUndo, canRedo }) => {
    ...
    useEffect(() => {
        // Deep compare files to check for changes
    ...
    }, [localFiles, files, onFilesChange, saveStatus]);
    ...
    return (
        <div className="bg-gray-900 flex flex-col flex-grow h-full">
            <div className="flex justify-between items-center p-2 px-4 bg-gray-800 border-b border-gray-700">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={onUndo} disabled={!canUndo} className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" aria-label="Undo change">
                        <UndoIcon aria-hidden="true" className="w-4 h-4" /> Undo
                    </button>
                    <button onClick={onRedo} disabled={!canRedo} className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" aria-label="Redo change">
                        <RedoIcon aria-hidden="true" className="w-4 h-4" /> Redo
                    </button>
                    <div aria-hidden="true" className="h-5 w-px bg-gray-600"></div>
                    <button onClick={handleExport} disabled={!localFiles || localFiles.length === 0} className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" aria-label="Export code as ZIP file">
                        <ExportIcon aria-hidden="true" className="w-4 h-4" /> Export
                    </button>
                    <button onClick={handleCopy} className="flex items-center gap-1.5 text-sm text-gray-300 hover:text-white bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white" aria-label="Copy code to clipboard">
            ...
            <div className="flex flex-grow overflow-hidden">
                <div className="w-48 bg-gray-800 border-r border-gray-700 p-2 overflow-y-auto">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider px-2 mb-2">Explorer</h3>
